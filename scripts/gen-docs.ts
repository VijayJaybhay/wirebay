...
import type { ConfigRead, Launch, ScopeName } from "../src/core/types.ts";
...
export class DocsGenerator {
  ...
  /**
   * Keep each guide's "Shared config files" section in sync with the manifests' `alsoReads`: which
   * other tools' files this tool reads, and which tools read this tool's files. The section is added
   * before the guide's Changelog the first time.
   */
  private async guides(tools: Tool[]): Promise<void> {
    const graph = new ConfigReadGraph(tools);
    const name = (id: string): string => tools.find((t) => t.id === id)?.name ?? id;
    const scope = (s: ScopeName): string => (s === "user" ? "global" : "project");
    const html = (s: string): string => s.replace(/</g, "&lt;").replace(/>/g, "&gt;");
    const when = (r: ConfigRead): string =>
      r.when === "setting" ? ` (when "${r.setting ?? "its setting"}" is on)` : r.when === "approval" ? " (after approval)" : "";
    for (const tool of tools) {
      const file = `tools/${tool.id}/GUIDE.md`;
      const abs = WirebayPaths.packagePath(file);
      if (!existsSync(abs)) continue;
      const reads = tool.alsoReads.map(
        (r) =>
          `- Reads **${name(r.tool)}'s ${scope(r.scope)}** config${when(r)}: ${html(r.note)}.${r.compatible ? "" : " It can't parse that file."} ([source](${r.source}))`,
      );
      const readBy = tool.scopes.flatMap((s) =>
        graph
          .readersOf(tool.id, s)
          .map(
            ({ reader, read }) =>
              `- Its **${scope(s)}** file is also read by **${reader.name}**${when(read)}: ${html(read.note)}.${read.compatible ? "" : ` **So wirebay only writes this file when you name ${tool.id} explicitly.**`} ([source](${read.source}))`,
          ),
      );
      const body = [
        "<!-- This section is generated by `npm run gen:docs` from `alsoReads` in the tool manifests. -->",
        "",
        ...(reads.length || readBy.length
          ? [
              ...reads,
              ...readBy,
              "",
      ...
      if (!text.includes("<!-- generated:reads -->")) {
        const section = "## Shared config files\n\n<!-- generated:reads -->\n<!-- /generated:reads -->\n\n";
        const at = text.indexOf("## Changelog");
        text = at >= 0 ? text.slice(0, at) + section + text.slice(at) : `${text.trimEnd()}\n\n${section}`;
      }
      await this.write(file, DocsGenerator.replaceBetween(text, "reads", body));
    }
  }
