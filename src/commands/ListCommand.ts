/**
 * `wirebay list [server|tool] [--global | --project | --dir <path>]`
 * @module
 */

import type { AppContext } from "../app/AppContext.ts";
import type { ParsedCommand } from "../cli/CommandParser.ts";
import { ExitCode } from "../core/errors.ts";
import { StateStore } from "../core/store/StateStore.ts";
import type { Entry, ScopeName } from "../core/types.ts";
import { Command } from "./Command.ts";
import { TargetSelector } from "./support/TargetSelector.ts";

/** The status of one server in one tool. */
type Cell = "synced" | "enabled" | "drift" | "stray" | "none";

/** One row of the matrix. */
interface Row {
  server: string;
  known: boolean;
  missingSecrets: string[];
  tools: Record<string, Cell>;
}

/** The matrix for one scope. */
interface ScopeReport {
  scope: ScopeName;
  label: string;
  /** Where this scope's desired state lives. */
  source: string;
  tools: string[];
  servers: Row[];
}

/**
 * Shows a server × tool matrix per scope (global, and this project when it has a `.wirebay.json`):
 * ✓ synced · ○ enabled but not synced · ~ edited by hand · ? synced but disabled · · off.
 */
export class ListCommand extends Command {
  readonly name = "list";
  override readonly aliases = ["ls", "status"];
  override readonly targeted = true;
  readonly help = {
    usage: "wirebay list [server|tool] [--global | --project | --dir <path>] [--json]",
    summary: "Show which servers are synced to which tools, globally and for this project.",
    examples: ["wirebay list", "wirebay ls codex", "wirebay list --project"],
  };

  run(input: ParsedCommand, ctx: AppContext): number {
    const t = ctx.terminal;
    const selector = new TargetSelector(ctx, input);
    const reports = selector.scopes().map((scope) => this.report(scope, input, ctx, selector));

    if (input.flags.json) {
      t.json(reports.length === 1 && reports[0] ? reports[0] : { scopes: reports });
      return ExitCode.Ok;
    }
    if (reports.every((r) => !r.servers.length)) {
      t.out("No servers added yet. Start with: wirebay add github to all   (or --project for this project only)");
      return ExitCode.Ok;
    }
    const symbol: Record<Cell, string> = {
      synced: t.ok("✓"),
      enabled: t.warn("○"),
      drift: t.warn("~"),
      stray: t.warn("?"),
      none: t.dim("·"),
    };
    for (const r of reports) {
      if (!r.servers.length) continue;
      t.out(`${t.bold(r.label)} ${t.dim(`(${r.source})`)}`);
      t.out(
        t.table(
          ["SERVER", ...r.tools, "SECRETS"],
          r.servers.map((row) => [
            row.known ? row.server : t.err(`${row.server} (unknown)`),
            ...r.tools.map((id) => symbol[row.tools[id] ?? "none"]),
            row.missingSecrets.length ? t.err(`! ${row.missingSecrets.join(", ")}`) : t.ok("ok"),
          ]),
        ),
      );
      t.out("");
    }
    t.out(t.dim("✓ synced  ○ enabled, run `wirebay sync`  ~ edited by hand  ? synced but disabled  · off"));
    return ExitCode.Ok;
  }

  private report(scope: ScopeName, input: ParsedCommand, ctx: AppContext, selector: TargetSelector): ScopeReport {
    const desired = ctx.desired.servers(scope);
    const state = ctx.state.load();
    const secretValues = ctx.secrets.all();
    const toolIds = Array.isArray(input.tools)
      ? input.tools
      : [...new Set([...desired.toolsInUse(), ...selector.toolsWrittenTo(scope, state)])].sort();
    const serverNames = Array.isArray(input.servers) ? input.servers.filter((s) => desired.has(s)) : desired.names();
    const files = this.readToolFiles(toolIds, scope, ctx);

    const cell = (server: string, tool: string): Cell => {
      const enabled = desired.toolsOf(server).includes(tool);
      const file = files.get(tool);
      const recorded = file?.recorded[server];
      const present = file?.entries[server];
      if (recorded && present) {
        if (ctx.hasher.hash(present) !== recorded) return "drift";
        return enabled ? "synced" : "stray";
      }
      return enabled ? "enabled" : "none";
    };

    const servers = serverNames.map((name): Row => {
      const known = ctx.servers.has(name);
      const missingSecrets = known
        ? ctx.servers
            .get(name)
            .requiredKeys()
            .filter((k) => !secretValues[k] && !ctx.env[k])
        : [];
      const tools: Record<string, Cell> = Object.fromEntries(toolIds.map((id) => [id, cell(name, id)]));
      return { server: name, known, missingSecrets, tools };
    });
    return { scope, label: selector.label(scope), source: ctx.desired.location(scope), tools: toolIds, servers };
  }

  /** Read each tool's file for the scope once: current entries and what wirebay recorded. */
  private readToolFiles(
    toolIds: string[],
    scope: ScopeName,
    ctx: AppContext,
  ): Map<string, { entries: Record<string, Entry>; recorded: Record<string, string> }> {
    const state = ctx.state.load();
    const out = new Map<string, { entries: Record<string, Entry>; recorded: Record<string, string> }>();
    for (const id of toolIds) {
      const tool = ctx.tools.get(id);
      const target = ctx.sync.targetFor(tool, scope);
      if (!target) continue;
      const recorded = StateStore.fileRecord(state, id, scope, target.file)?.entries ?? {};
      let entries: Record<string, Entry> = {};
      try {
        entries = ctx.adapters.for(tool).read(target).entries;
      } catch {
        // Unreadable file: shown as not synced.
      }
      out.set(id, { entries, recorded });
    }
    return out;
  }
}
