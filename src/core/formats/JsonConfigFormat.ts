...
export class JsonConfigFormat implements ConfigFormat {
  read(text: string, rootKey: string, file: string): ReadResult {
    ...
    let node = JsonConfigFormat.parseOrThrow(text, file) as Record<string, unknown> | undefined;
    for (const k of splitRootKey(rootKey)) node = node && typeof node === "object" ? (node[k] as Record<string, unknown> | undefined) : undefined;
    const entries = node && typeof node === "object" && !Array.isArray(node) ? (node as Record<string, Entry>) : {};
    return { entries, locked: new Set() };
  }

  write(text: string, rootKey: string, changes: Changes, file: string): string {
    const style = JsonConfigFormat.detectStyle(text);
    let out = text.trim() ? text : `{}${style.eol}`;
    JsonConfigFormat.parseOrThrow(out, file);
    const base = splitRootKey(rootKey);

    for (const name of changes.remove) {
      const node = findNodeAtLocation(parseTree(out)!, [...base, name]);
      if (node) out = JsonConfigFormat.removeProperty(out, node);
    }
    for (const [name, entry] of Object.entries(changes.set)) {
      out = this.setProperty(out, base, name, entry, style, file);
    }
  ...
  private setProperty(text: string, base: string[], name: string, entry: Entry, style: Style, file: string): string {
    ...
    if (findNodeAtLocation(root, [...base, name])) {
      return applyEdits(text, modify(text, [...base, name], entry, { formattingOptions: JsonConfigFormat.formatting(style) }));
    }
    ...
    return JsonConfigFormat.insertProperty(text, parent, rest[0]!, value, style);
  }

  private static parseOrThrow(text: string, file: string): unknown {
  ...
  private static detectStyle(text: string): Style {
    const eol = text.includes("\r\n") ? "\r\n" : "\n";
    const m = text.match(/\n([ \t]+)["{\[]/);
    return { eol, unit: m?.[1] ?? "  " };
  }

  private static formatting(style: Style): FormattingOptions {
    const tabs = style.unit.includes("\t");
    return { insertSpaces: !tabs, tabSize: tabs ? 1 : style.unit.length, eol: style.eol };
  }

  /** Leading whitespace of the line containing `offset`. */
  private static lineIndent(text: string, offset: number): string {
    const start = text.lastIndexOf("\n", offset - 1) + 1;
    return text.slice(start).match(/^[ \t]*/)![0];
  }

  private static pretty(value: unknown, indent: string, style: Style): string {
    return JSON.stringify(value, null, style.unit).replace(/\n/g, style.eol + indent);
  }

  /** Insert `"key": value` as the last property of an object node. */
  private static insertProperty(text: string, obj: Node, key: string, value: unknown, style: Style): string {
    const children = obj.children ?? [];
    if (children.length) {
      const last = children[children.length - 1]!;
      const indent = JsonConfigFormat.lineIndent(text, children[0]!.offset);
      const at = last.offset + last.length;
      const prop = `${JSON.stringify(key)}: ${JsonConfigFormat.pretty(value, indent, style)}`;
    ...
    const outer = JsonConfigFormat.lineIndent(text, obj.offset);
    const indent = outer + style.unit;
    const inner = `${style.eol}${indent}${JSON.stringify(key)}: ${JsonConfigFormat.pretty(value, indent, style)}${style.eol}${outer}`;
  ...
  private static removeProperty(text: string, valueNode: Node): string {
