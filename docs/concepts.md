...
3. **Sync** a small *launcher entry* into each tool. The entry contains no secrets; it just says
...
| Term | Meaning |
|---|---|
| **Server** | An MCP server, like GitHub or Netlify. Defined by a JSON *definition*: how to start it and which secrets it needs. |
...
| **Tool** (or client) | An AI app that uses MCP servers: Claude Code, Codex, Cursor… Described by a *manifest* in the tools directory. |
...
| **Export** | Write the files wirebay *would* produce into `./wirebay-export/` without touching real configs. |
