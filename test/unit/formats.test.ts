...
import { splitRootKey, type Changes } from "../../src/core/formats/ConfigFormat.ts";
...
const set = (entries: Record<string, Record<string, unknown>>): Changes => ({ set: entries, remove: [] });
const remove = (...names: string[]): Changes => ({ set: {}, remove: names });
...
await test("root keys split on dots, and \\. is a literal dot", () => {
  assert.deepEqual(splitRootKey("mcp.servers"), ["mcp", "servers"]);
  assert.deepEqual(splitRootKey("amp\\.mcpServers"), ["amp.mcpServers"]);
});

await test("json: add then remove round-trips byte for byte", () => {
...
await test("json: replace an existing entry", () => {
...
await test("json: creates the file, nested keys, and flat dotted keys", () => {
  assert.deepEqual(JSON.parse(json.write("", "mcpServers", set({ a: { command: "x" } }), "f")), { mcpServers: { a: { command: "x" } } });
  assert.deepEqual(JSON.parse(json.write('{\n  "x": 1\n}\n', "mcp.servers", set({ a: { command: "x" } }), "f")), {
    x: 1,
    mcp: { servers: { a: { command: "x" } } },
  });
  assert.deepEqual(JSON.parse(json.write("", "amp\\.mcpServers", set({ a: { command: "x" } }), "f")), {
    "amp.mcpServers": { a: { command: "x" } },
  });
});

await test("json: removing the middle, first, last and only entries", () => {
  const three = '{\n  "s": {\n    "a": 1,\n    "b": 2,\n    "c": 3\n  }\n}\n';
...
await test("json: keeps CRLF and tab indentation", () => {
  const crlf = '{\r\n\t"mcpServers": {\r\n\t\t"a": {}\r\n\t}\r\n}\r\n';
...
await test("toml: managed block leaves the rest byte for byte and removes cleanly", () => {
...
await test("toml: rewriting the block keeps text after it", () => {
...
await test("yaml: edits keep comments; new files are block style", () => {
  const y = "# top\nmcpServers:\n  mine:\n    command: bar # inline\n";
