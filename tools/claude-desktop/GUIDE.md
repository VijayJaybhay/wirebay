...
- Its **global** file is also read by **Visual Studio Code (GitHub Copilot agent mode)** (when "chat.mcp.discovery.enabled" is on): discovers Claude Desktop's servers when MCP discovery is on. ([source](https://code.visualstudio.com/docs/agent-customization/mcp-servers))
